...
class Vacancy:
    ...
    def __post_init__(self) -> None:
        ...
        # средняя/эффективная зарплата для сортировки
    ...
    def from_hh(cls, item: dict[str, Any]) -> "Vacancy":
        ...
        req = str(item.get("snippet", {}).get("requirement") or "")
        resp = str(item.get("snippet", {}).get("responsibility") or "")
        desc = f"{req} {resp}".strip()
        ...
        return cls(id=vid, title=name, url=url, description=desc,
                   salary_from=s_from, salary_to=s_to, currency=cur)
